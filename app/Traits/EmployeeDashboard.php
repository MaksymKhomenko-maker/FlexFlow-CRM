...
trait EmployeeDashboard
{
    ...
    public function employeeDashboard()
    {
        ...
        $utcOffset = Carbon::now(company()->timezone)->format('P');

        $this->currentClockIn = Attendance::whereDate(
                DB::raw('CONVERT_TZ(clock_in_time, "+00:00", "'. $utcOffset.'")'),
                $currentDate
            )
            ->where('user_id', $this->user->id)
            ->whereNull('clock_out_time')
        ...
        $this->checkTodayHoliday = Holiday::where('date', $currentDate)
            ...
            ->where(function ($query) use ($user) {
                if(!is_Null($user->employeeDetails->employment_type )){
    ...
    public function attendanceShift($defaultAttendanceSettings)
    {
        ...
        $nowTime = now($this->company->timezone);
