...
class TimelogController extends AccountBaseController
{
    ...
    public function employeeData(Request $request)
    {
        ...
        $timeSheetStartDate = Carbon::createFromFormat($this->company->date_format, $timeSheetStartDate)->toDateString();
        $timeSheetEndDate = Carbon::createFromFormat($this->company->date_format, $timeSheetEndDate)->toDateString();

        $employee = $request->employee;
        $projectId = $request->projectID;
        $this->viewTimelogPermission = user()->permission('view_timelogs');

        $query = User::join('employee_details', 'users.id', '=', 'employee_details.user_id')
        ...
        $query->leftJoin('project_time_logs', function ($join) use ($timeSheetStartDate, $timeSheetEndDate, $where) {
            $join->on('project_time_logs.user_id', '=', 'users.id')
                ->whereRaw('DATE(project_time_logs.start_time) >= ?', [$timeSheetStartDate])
                ->whereRaw('DATE(project_time_logs.start_time) <= ?', [$timeSheetEndDate])
        ...
        $query->select([
            ...
            ->leftJoin('project_time_log_breaks', function ($join) use ($timeSheetStartDate, $timeSheetEndDate, $where) {
